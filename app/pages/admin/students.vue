    ...
    <div class="p-6 space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-400 mt-0.5">Manage all student accounts</p>
            </div>
        ...
        <div v-if="loading" class="bg-white rounded-xl p-12 border border-gray-100 text-center">
            ...
            <p class="text-sm text-gray-400 mt-3">Loading students...</p>
        </div>
        ...
        <div v-else-if="filtered.length === 0" class="bg-white rounded-xl p-12 border border-gray-100 text-center">
            ...
            <p class="text-base font-semibold text-gray-700">No students found</p>
            <p class="text-sm text-gray-400 mt-1">{{ searchQuery || statusFilter ? 'Try adjusting your filters.' : 'No students have registered yet.' }}</p>
        </div>
        ...
        <div v-else class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <tr v-for="u in filtered" :key="u.id" class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="py-4 px-5">
                            <button @click="openCoursesModal(u)" class="text-sm text-[#0000ff] font-medium hover:underline">
                                {{ u._count.Enrollment }} course{{ u._count.Enrollment !== 1 ? 's' : '' }}
                            </button>
                        </td>
            ...
            <div class="px-5 py-3 border-t border-gray-100 bg-gray-50/30">
                <p class="text-xs text-gray-400">Showing {{ filtered.length }} of {{ users.length }} student{{ users.length !== 1 ? 's' : '' }}</p>
            </div>
        </div>
        ...
        <Teleport to="body">
            <div v-if="coursesModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="coursesModal = null">
                ...
                <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                    ...
                    <div v-else-if="userEnrollments.length === 0" class="py-8 text-center text-sm text-gray-400">Not enrolled in any courses.</div>
                    <div v-else class="space-y-2 max-h-72 overflow-y-auto">
                        <div v-for="e in userEnrollments" :key="e.id" class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            <div>
                                <p class="text-sm font-semibold text-gray-900">{{ e.Course.title }}</p>
                                <p class="text-xs text-gray-400 font-mono">{{ e.Course.code }}</p>
                            </div>
                            <span :class="enrollStatusBadge(e.status as string).cls">{{ enrollStatusBadge(e.status as string).label }}</span>
                        </div>
                    </div>
                </div>
            </div>
...
import { Search, Users, Check, X, Ban } from 'lucide-vue-next'
...
interface Enrollment {
    ...
    status: string
    enrolledAt: string
    Course: { id: number; title: string; code: string; duration: string | null }
}
...
const fetchUsers = async () => {
    loading.value = true
    try {
        users.value = await $fetch<User[]>('/api/admin/users', { query: { role: 'STUDENT' } })
    } finally {
        loading.value = false
    }
}
...
const deactivate = async (id: number) => {
    if (!confirm('Deactivate this student account? They will no longer be able to log in.')) return
    actionId.value = id
    try {
...
const openCoursesModal = async (u: User) => {
    ...
    try {
        userEnrollments.value = await $fetch<Enrollment[]>(`/api/admin/users/${u.id}/enrollments`)
    } finally {
...
const enrollStatusBadge = (status: string) => {
    if (status === 'ACTIVE') return { label: 'Enrolled', cls: 'text-xs font-semibold px-2 py-0.5 rounded-full bg-green-50 text-green-600' }
    if (status === 'PENDING') return { label: 'Pending', cls: 'text-xs font-semibold px-2 py-0.5 rounded-full bg-orange-50 text-orange-600' }
    if (status === 'REJECTED') return { label: 'Rejected', cls: 'text-xs font-semibold px-2 py-0.5 rounded-full bg-red-50 text-red-600' }
    return { label: status, cls: 'text-xs font-semibold px-2 py-0.5 rounded-full bg-gray-100 text-gray-500' }
}
</script>
